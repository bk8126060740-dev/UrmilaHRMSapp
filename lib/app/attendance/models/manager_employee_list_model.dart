...
//
//     final managerEmployeeListModel = managerEmployeeListModelFromJson(jsonString);

import 'dart:convert';

ManagerEmployeeListModel managerEmployeeListModelFromJson(String str) =>
    ManagerEmployeeListModel.fromJson(json.decode(str));

String managerEmployeeListModelToJson(ManagerEmployeeListModel data) =>
    json.encode(data.toJson());

class ManagerEmployeeListModel {
  List<EmployeeListItem>? employeeList;

  ManagerEmployeeListModel({this.employeeList});

  ManagerEmployeeListModel copyWith({List<EmployeeListItem>? employeeList}) =>
      ManagerEmployeeListModel(employeeList: employeeList ?? this.employeeList);

  factory ManagerEmployeeListModel.fromJson(Map<String, dynamic> json) =>
      ManagerEmployeeListModel(
        employeeList: json["list"] == null
            ? []
            : List<EmployeeListItem>.from(
                json["list"]!.map((x) => EmployeeListItem.fromJson(x)),
              ),
      );
